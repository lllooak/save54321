-- FIX: Use correct transaction types that match check constraint
-- earning (not earnings) and fee (not platform_fee)
...
BEGIN
  ...
  -- Check if earnings transaction exists - FIXED: Use 'earning' not 'earnings'
  SELECT EXISTS(
    SELECT 1 FROM wallet_transactions 
    WHERE reference_id = p_request_id::text 
    AND type = 'earning'  -- FIXED: singular form
  ...
  IF NOT v_earnings_transaction_exists THEN
    ...
    -- Create wallet transaction for creator earnings - FIXED: Use 'earning' not 'earnings'
    INSERT INTO wallet_transactions (
      ...
      gen_random_uuid(),
      v_creator_id,
      'earning',  -- FIXED: Use singular form that matches constraint
      v_creator_earnings,
  ...
  -- Check if platform fee transaction exists - FIXED: Use 'fee' not 'platform_fee'
  SELECT EXISTS(
    SELECT 1 FROM wallet_transactions 
    WHERE reference_id = p_request_id::text 
    AND type = 'fee'  -- FIXED: Use 'fee' instead of 'platform_fee'
    AND user_id = v_creator_id
  ) INTO v_platform_fee_transaction_exists;

  -- Create platform fee transaction only if doesn't exist - FIXED: Use 'fee' not 'platform_fee'
  IF NOT v_platform_fee_transaction_exists THEN
    INSERT INTO wallet_transactions (
      ...
      gen_random_uuid(),
      v_creator_id,
      'fee',  -- FIXED: Use 'fee' that matches constraint
      -v_platform_fee,
