-- Fix for video upload error: Update RPC function to accept 'approved' status
-- The issue is that requests have status 'approved' but function expects 'pending'
...
BEGIN
  ...
  AND r.status IN ('pending', 'approved');  -- FIXED: Accept both statuses
  ...
  INSERT INTO wallet_transactions (
    ...
    'platform_fee',
    -v_platform_fee,
    'Platform fee for request: ' || p_request_id,
    p_request_id,
    NOW()
  ) ON CONFLICT (reference_id, type, user_id) DO NOTHING;
