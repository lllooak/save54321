...
AS $$
DECLARE
  v_creator_id uuid;      -- Renamed with v_ prefix
  v_fan_id uuid;          -- Renamed with v_ prefix
  ...
  v_result json;
BEGIN
  -- Get request details with table prefix to avoid ambiguity
  SELECT 
    r.creator_id, 
    r.fan_id, 
    r.amount
  INTO v_creator_id, v_fan_id, v_request_amount
  FROM requests r
  ...
  -- Calculate amounts
  v_platform_fee := v_request_amount * 0.30;
  v_creator_earnings := v_request_amount - v_platform_fee;
...
SELECT 'AMBIGUOUS COLUMNS FIXED - READY FOR TESTING' as final_status;
