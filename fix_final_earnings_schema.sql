-- FINAL FIX: Match EXACT earnings table schema
-- Earnings table only has: creator_id, request_id, amount, status
-- Remove both fan_id AND platform_fee from earnings insert
-- Fix all UUID/text casting issues
...
BEGIN
  ...
  -- Create earnings record - FIXED: Only use columns that exist in table
  -- earnings table only has: creator_id, request_id, amount, status
